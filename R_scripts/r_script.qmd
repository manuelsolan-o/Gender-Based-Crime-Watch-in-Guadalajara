...
ui <- fluidPage(
    titlePanel("Visualización de Datos de Crimen"),
    sidebarLayout(
        sidebarPanel(
            selectInput("crimenInput", "Seleccione un Tipo de Crimen:",
                        choices = c('Robo a negocio', 'Robo a int de vehiculos', 'Lesiones dolosas',
                                    'Robo a vehiculos particulares', 'Homicidio doloso', 'Violencia familiar',
                                    'Robo casa habitacion', 'Robo a persona', 'Abuso sexual infantil',
                                    'Robo de motocicleta', 'Robo a cuentahabientes', 'Robo a carga pesada',
                                    'Robo de autopartes', 'Violacion', 'Robo a bancos', 'Feminicidio')),
        actionButton("submit", "Actualizar Mapa")
        ),
        mainPanel(
            leafletOutput("map")
        )
...
server <- function(input, output) {
  ...
  paste(capture.output(print(xtable(info), type = "html", include.rownames = TRUE)), collapse = "\n")
    ...
    # Observador para el botón de actualizar
    observeEvent(input$submit, {
        # Filtrar los datos según el crimen seleccionado
        filtered_data <- data %>%
        ...
        output$map <- renderLeaflet({
            ...
            m <- leaflet(filtered_data) %>% 
                ...
                setView(lng = -103.39182, lat = 20.66682, zoom = 12) %>%
                addCircleMarkers(
                    lng = filtered_data$x, lat = filtered_data$y,
                    radius = 3,
                    color = color_dict[[input$crimenInput]],
                    fillOpacity = 1,
                    opacity = 1,
                    popup = lapply(1:nrow(filtered_data), function(i) {
                        createPopupHTML(filtered_data[i, ])
                    })
                )
            m
        })
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            selectInput("crimenInput", "Seleccione un Tipo de Crimen:",
                        choices = c('Robo a negocio', 'Robo a int de vehiculos', ...)),  # Continúa con las opciones
            actionButton("submit", "Actualizar Mapa")
        ),
        mainPanel(
            leafletOutput("map")
